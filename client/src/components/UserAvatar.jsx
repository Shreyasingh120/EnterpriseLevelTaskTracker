...
import { useLogoutMutation } from "../redux/slices/api/authApiSlice"; // <-- update the path as needed
...
const UserAvatar = () => {
  ...
  const logoutHandler = async () => {
    try {
      await logoutUser().unwrap();         // 1. Call API to clear cookie/session
      dispatch(logout());                  // 2. Clear Redux state + localStorage
      navigate("/log-in");                 // 3. Redirect to login page
    } catch (error) {
